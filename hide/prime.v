...
Definition lt_rect f rec x := lt_wf_rect x f rec. 
...
Section PrimeDec.
  ...
  Proof.
    ...
    destruct (prime_or_div N) as [| [q (( H1 & H2 ) & H3) ] ]; auto.
    - exists N. split; auto. 
      exists 1; lia.
...
Definition factorization (n : nat) : n <> 0 ->
  { l : list nat | n = list_prod l /\ Forall prime l }.
Proof. 
  intros Hn.
  induction n as [n rec] using lt_rect.
  destruct n as [|[|n]]; try tauto.
  - exists nil; auto.
  - assert (H: S (S n) > 1) by lia.
    destruct (prime_factor _ H) as (q & prime_q & [x Hx]%Witness).
    destruct (rec x) as (lx & Hlx & ?).
    + destruct x, q, prime_q; lia.
    + lia.
    + exists (q :: lx). split.
      * now rewrite <-Hx, Hlx.
      * now constructor.   
