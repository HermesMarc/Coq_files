...
(*
  Right now this formalizes a coding number which can be extended by one number at the time.
  But the Cons1 property really looks like something that is close to a prime property.
  So its conclusion could be u ∈ c1 \/ u ∈ c2 given that u ∈ (c1 ++ c2).
  ++ would then need to be an operation which combines two codes to a new code.
...
(*
  ...
  I get the feeling that this can therefore be tied more intemetely to
  recursion as the general principle, In the sense that being able to
  do coding is eqivalent to being able to do recursion. This would align
  with what is happening in the Gödel incompleteness proofs. Here one
  needs coding of sequences in order to formulate formulas for (primitive)
  recursive functions.
 *)

Fact Cons1 n c x :  x ∈ n ;; c -> x = n \/ x ∈ c.
Proof. firstorder. Qed.

Fact Cons2 n c x :  x ∈ c -> x ∈ n ;; c.
Proof. firstorder. Qed.

Fact Cons3 n c :  n ∈ n ;; c.
Proof. firstorder. Qed.

Hint Resolve Cons3 Cons2 : core.
...
Proof.
  ...
  - intros Dec_p.
    ...
    + exists (n ;; c).
      ...
      ++ intros [| ->]%E. split.
        * intros p_u%Hc; auto.
        * intros [->|]%Cons1; auto. now apply Hc.
        * split; eauto.
      ++ intros [->|H%Hc]%Cons1; auto.
    + exists c.
      intros u; split.
